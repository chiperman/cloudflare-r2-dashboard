...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    const { data: deletedRecords, error: dbError } = await deleteQuery.select();
    ...
    // R2 deletion logic remains the same, it operates on what was successfully deleted from DB
    ...
    if (objectsToDelete.length > 0) {
      ...
      const { Errors } = await s3Client.send(deleteCommand);
